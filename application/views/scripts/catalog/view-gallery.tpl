...
{if_allowed resource="{$controller}/add"}
    <div class="sub-menu">
        <img src="/i/add.png"/>&nbsp;<a href="{$this->url(['controller' => $controller,'action' => 'addGallery', 'idProduct' => $product->id, 'rubric' => $cur_rubric->getId()])}">добавить фото</a>
    </div>
{/if_allowed}
                ...
                <li>
                    ...
                    <ul id="gallery_action_{$gallery->id}" class="gallery_action_menu">
                        ...
                        {if_allowed resource="{$controller}/editGallery"}
                            <li class="action"><img src="/i/edit.png"/>&nbsp;<a href="{$this->url(['controller' => $controller,'action' => 'editGallery', 'idProduct' => $product->id, 'id' => $gallery->id, 'rubric' => $cur_rubric->getId()])}">редактировать</a></li>
                        {/if_allowed}

                        {if_allowed resource="{$controller}/deleteGallery"}
                            <li class="action"><img src="/i/delete.png"/>&nbsp;<a href="{$this->url(['controller' => $controller,'action' => 'deleteGallery', 'idProduct' => $product->id, 'id' => $gallery->id, 'rubric' => $cur_rubric->getId()])}" onclick="return confirmDelete('{$gallery->title}');" style="color: #830000">удалить</a></li>
                        {/if_allowed}
                    </ul>
                </li>
        {/foreach}
